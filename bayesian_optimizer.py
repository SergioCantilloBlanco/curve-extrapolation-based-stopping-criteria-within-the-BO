...
class BayesianOptimizer:
    ...
    def step(self):
        ...
        y = np.array(self.recorded_points.y)
    ...
    def oned_plot(self, y_pred, y_std,new_x,new_y,i):
        ...
        ax1.scatter(self.recorded_points.X, self.recorded_points.y, color='red', label='Samples')
        ...
        ax2.plot(np.arange(len(self.recorded_points.best_y)), self.recorded_points.best_y,color='orange', label='convergence curve')
        ax2.scatter(np.arange(len(self.recorded_points.best_y)), self.recorded_points.best_y, color='blue', label='samples')
    ...
    def multid_plot(self, y_pred, y_std,new_x,new_y,optimal_values,i):
        ...
        X = np.array(self.recorded_points.X)
        ax1.scatter(X[:, 0], self.recorded_points.y, color='red', label='Samples')
        ax1.scatter([new_x[0]], [new_y], color='blue', label='New point')
        ax1.set_title(f'Iteration {i}')
        ax1.legend()

        extrapolated_curve_x = np.linspace(0,len(self.recorded_points.best_y)+5,200)
        extrapolated_curve_y = exp_decay(extrapolated_curve_x, *optimal_values)
        ax2.plot(self.recorded_points.best_y, color='orange', label="convergence curve")
    ...
    def loop(self):
        for i in range(self.num_iter):
            ...
            x_data_convergence_curve = np.arange(len(self.recorded_points.best_y))
            a = self.recorded_points.best_y[0] - self.recorded_points.best_y[-1]
            b = 0.1
            c = self.recorded_points.best_y[-1]
            try:
